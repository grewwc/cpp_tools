...
public:
    ...
    template <typename Container> static String join(const char *delim, Container &&data)
    {
        if (data.size() == 0) return String{""};
        std::ostringstream os;
        const auto delim_len = strlen(delim);
        for (auto &&each_str : std::forward<Container>(data)) { os << each_str << delim; }
        auto temp = os.str();
    ...
    String operator*(const size_t N) const noexcept
    {
        ...
        const size_t SIZE = this->size();
        res.resize(N * SIZE);
        for (size_t i = 0; i < N; i++) { std::copy(this->cbegin(), this->cend(), res.begin() + i * SIZE); }
        return res;
    }
...
private:
    template <typename T> std::vector<T> find_all_number(char type) const noexcept
    {
